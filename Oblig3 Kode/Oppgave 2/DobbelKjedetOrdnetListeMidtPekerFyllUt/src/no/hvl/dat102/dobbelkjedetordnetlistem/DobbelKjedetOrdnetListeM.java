...
public class DobbelKjedetOrdnetListeM<T extends Comparable<T>> implements DobbelKjedetOrdnetListeMADT<T> {
	...
	public T fjern(T el) {
		 ...
		 * (el.compareTo(siste.getElement()) >= 0)) { // Ugyldig. Alternativt
		 * kan vi ha det som et forkrav! System.out.println(
		 * "Ugyldig verdi. verdi > " + foerste.getElement() + "verdi < " +
		 * siste.getElement());
		 * 
		 * } else { // Kun lovlige verdier
		 * 
		 ...
		 * = foerste.getNeste(); }
		 * 
		 * while (el.compareTo(p.getElement()) > 0) { p = p.getNeste(); } //
		 * while
		 * 
		...
		if (p != null) {
			...
			// Oppadtere midten
			if (antall%2 == 0 || midten.getElement().equals(resultat)) {
			nyMidten();
			}
